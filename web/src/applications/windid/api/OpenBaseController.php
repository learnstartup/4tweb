...
class OpenBaseController extends PwBaseController {
	...
	public  function beforeAction($handlerAdapter) {
		...
		//print_r(WindidUtility::appKey($clent['id'], $_time, $clent['secretkey']));die;
		if (WindidUtility::appKey($clent['id'], $_time, $clent['secretkey']) != $_windidkey)  $this->output(WindidError::FAIL);
		$time = Windid::getTime();
		if ($time - $_time > 1200000) $this->output(WindidError::TIMEOUT);
	...
	protected function class_object_to_array($stdclassobject)
    {
        $_array = is_object($stdclassobject) ? get_object_vars($stdclassobject) : $stdclassobject;

        foreach ($_array as $key => $value) {
            $value = (is_array($value) || is_object($value)) ? $this->class_object_to_array($value) : $value;
            $array[$key] = $value;
        }

...
<body>
	...
	<ul class="cc">
		...
		<div class="search_type cc mb10">
			<form class="" action="<?php echo Wekit::app()->baseUrl,'/','admin.php?id&app=4tschool&m=app&c=shopstatusrecord'; ?>" method="post">
				...
				<input type="hidden" class="pageurl" name="pageurl" value="<?php echo Wekit::app()->baseUrl,'/','admin.php?app=4tschool&m=app&c=shopstatusrecord'; ?>">
				<label>商店ID:</label>	
		...
		<div class="table_list">
			<table width="100%">
				...
				<?php  if ($shopStatusRecordList) { 
 foreach ($shopStatusRecordList as $key => $item) {?>
				<tr>
					<td><?php echo htmlspecialchars($item['schoolname'], ENT_QUOTES, 'UTF-8');?></td>
					<td><?php echo htmlspecialchars($item['shopname'], ENT_QUOTES, 'UTF-8');?></td>
					<td><?php echo htmlspecialchars($item['userId'], ENT_QUOTES, 'UTF-8');?></td>
					<td><?php echo htmlspecialchars($item['actiontime'], ENT_QUOTES, 'UTF-8');?></td>
					<td><?php echo htmlspecialchars($item['actionstatus']==1?'开':'关', ENT_QUOTES, 'UTF-8');?></td>
				</tr>
				<?php }
  } else { ?>
				<tbody>
					<tr><td colspan="14" class="tac">啊哦，目前没有记录哟！</td></tr>
				</tbody>
				<?php  } ?>
			</table>
		</div>
...
<script>
	...
	{
		var schoolpara = '&chooseschool='+$(".chooseschool").val();
		var shoppara = '&choosenShopid='+$(".choosenShopid").val();
		if(!showP)
		{
			provincepara = "";	
		}
		window.location.href= $(".pageurl").val()+schoolpara+shoppara; 
	}
</script>
</body>
</html>
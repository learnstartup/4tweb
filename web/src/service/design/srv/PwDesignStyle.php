...
class PwDesignStyle {
			...
			switch ($k)	{
				...
				case 'border':
					if (!$v['isdiffer']){
						...
						foreach ($v AS $_k=>$_v) {
							if (!in_array($_k, array('top','left','right','bottom'))) continue;
	...
	public function getLink($style = array()) {
		$this->_style = array();
		if ($style['link']) $this->_setFont($style['link']['size'], $style['link']['color']);
	...
	private function _setBorder($width = null, $line = null, $color = null,$tblr = null) {
		$style = '';
		if ($tblr) $tblr .= '-';
		if (isset($width) && $width != '') $style .= 'border-'.$tblr.'width: '.$width.'px;';
		if ($line) $style .= 'border-'.$tblr.'style: '.$line.';';
		if ($color) $style .= 'border-'.$tblr.'color: '.$color.';';
		$this->_appendStyle($style);
	}
	
	private function _setMargin($top = 0, $right = 0,$bottom = 0, $left = 0) {
		$style = '';
		if ($top || $right || $bottom || $left) $style .= 'margin: '.$top .'px '. $right.'px '.$bottom.'px '.$left.'px;';
		$this->_appendStyle($style);
	}
	
	private function _setPadding($top = 0, $right = 0,$bottom = 0,$left = 0) {
		$style = '';
		if ($top || $right || $bottom || $left) $style .= 'padding: '.$top .'px '. $right.'px '.$bottom.'px '.$left.'px;';
		$this->_appendStyle($style);
	}
	...
	private function _setBackground($color = null, $backimage = null, $position = null) {
		...
		if (in_array($position, array('left', 'right', 'center'))) $style .= 'background-position: '.$position.' top;background-repeat:no-repeat;';
		if ($position == 'repeat') $style .= 'background-repeat:repeat-x;background-position: left top;';
		$this->_appendStyle($style);
	}
	...
	private function _setFloat($type = 'left', $pixels = 0) {
		$style = '';
		$pixels = (int)$pixels;
		if ($type == 'left') $style .= 'float:left;margin-left:'.$pixels.'px;';
		if ($type == 'right') $style .= 'float:right;margin-right:'.$pixels.'px;';
		$this->_appendStyle($style);
	}

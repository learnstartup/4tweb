...
class PwCronService {
	...
	public function getNextTime($loopType, $day = 0, $hour = 0, $minute = 0) {
		$time = Pw::getTime();
		if ($timezone = Wekit::C('site', 'time.timezone')) $time += $timezone * 3600;
		...
		if ($timezone) $nexttime -= $timezone * 3600;
		return $nexttime;
	}
	...
	public function runCron() {
		...
		if (!$cron || $cron['next_time'] > $_time) return false;
		list($day, $hour, $minute) = explode('-', $cron['loop_daytime']);
	...
	private function _runAction($filename = '', $cronId = 0) {
		if (!$filename || Pw::substrs($filename,8,0,false) != 'PwCronDo') return false;

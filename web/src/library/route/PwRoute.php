			...
			if (!empty($rule)) {
				...
				foreach ($rule as $k => $v) {
					...
					if (preg_match($v['pattern'], $rewritePath, $matches)) {
						...
						}else if($k == 'shopdetails')
						{
							$args = WindUrlHelper::urlToArgs(trim($queryPath, '?'), true);
							$matches = array_merge($matches,array("app" =>"4tschool",'m'=>'app','c'=>'shopdetails','a'=>'run'),$args);
							return $matches;
						}
						else if($k == 'shopphone')
						{
							$args = WindUrlHelper::urlToArgs(trim($queryPath, '?'), true);
							$matches = array_merge($matches,array("app" =>"4tschool",'m'=>'app','c'=>'shopdetails','a'=>'showphone'),$args);
							return $matches;
						}
						else if($k == 'mdetails')
						{
							$args = WindUrlHelper::urlToArgs(trim($queryPath, '?'), true);
							$matches = array_merge($matches,array("app" =>"4tschool",'m'=>'app','c'=>'shopdetails','a'=>'mdetail'),$args);
							return $matches;
						}
						else if($k == 'indexrun')
						{
							$args = WindUrlHelper::urlToArgs(trim($queryPath, '?'), true);
							$matches = array_merge($matches,array("app" =>"4tschool",'m'=>'app','c'=>'index','a'=>'run'),$args);
							return $matches;
						}

 ...
 * @version $Id: ManageController.php 23489 2013-01-10 05:56:12Z xiaoxia.xuxx $
 * @package 
 */
class ManageController extends AdminBaseController {
	...
	public function getcodeAction() {
			...
			{
				$returnData['data'] = $result->getError();
				$returnData['success'] = false;
				print_r(json_encode($returnData));
				die;
			}
			...
			{
				$pwmobileVerify = Wekit::load('mobile.PwUserMobileVerify');
				//get code to admin
				$mobilecode = $pwmobileVerify->getMobileVerify($mobile);

				if(empty($mobilecode))
				{
					$returnData['data'] = "未发现手机验证码";
					$returnData['success'] = false;
					print_r(json_encode($returnData));
					die;
				...
				else
				{
					...
					$this->setOutput($code,"code");
					$this->setOutput("ignore","sendcode");
					$this->setOutput(true,"codevisible");

					$returnData['data'] = "验证码是:".$code;

...
class PwPostDoAtt extends PwPostDoBase {
	...
	function editAttachs($oldatt_desc, $oldatt_cost, $oldatt_ctype) {
	...
	public function dataProcessing($postDm) {
		...
		if ($ifupload & 1) $postDm->setHasImage(true);
		if ($ifupload & 2) $postDm->setHasTxt(true);
		if ($ifupload & 4) $postDm->setHasZip(true);
		return $postDm;
	}

...
<div class="tac"> </div>
	<div class="main_wrap">
		...
		<div class="bread_crumb">
			<?php echo $headguide;
  if ($tab == 'digest') { ?><em>&gt;</em><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=cate&fid=', rawurlencode($fid),'&tab=digest'; ?>">精华</a><?php }?>
		</div>
	...
	<?php  } ?>
					...
					<div class="box_wrap thread_page J_check_wrap">
						...
						<div class="content_nav" id="hashpos_ttype">
							...
							<div class="content_filter"><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=cate&fid=', rawurlencode($fid); 
 echo htmlspecialchars($_urladd_, ENT_QUOTES, 'UTF-8');
 echo htmlspecialchars($defaultOrderby == 'lastpost' ? '&orderby=postdate' : '', ENT_QUOTES, 'UTF-8');?>"  class="<?php echo htmlspecialchars(Pw::isCurrent($orderby == 'postdate'), ENT_QUOTES, 'UTF-8');?>">最新发帖</a><span>|</span><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=cate&fid=', rawurlencode($fid); 
 echo htmlspecialchars($_urladd_, ENT_QUOTES, 'UTF-8');
 echo htmlspecialchars($defaultOrderby == 'lastpost' ? '' : '&orderby=lastpost', ENT_QUOTES, 'UTF-8');?>" class="<?php echo htmlspecialchars(Pw::isCurrent($orderby != 'postdate'), ENT_QUOTES, 'UTF-8');?>">最后回复</a></div>
							<ul class="cc">
								<li class="<?php echo htmlspecialchars(Pw::isCurrent(!$tab), ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=cate&fid=', rawurlencode($fid); ?>">全部</a></li>
								<li class="<?php echo htmlspecialchars(Pw::isCurrent('digest' == $tab), ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=cate&fid=', rawurlencode($fid),'&tab=digest'; ?>">精华</a></li>
							</ul>
						</div>
						</nav>
						<?php  if ($threaddb) { ?>
						<div class="thread_posts_list">
							<table width="100%" id="J_posts_list" summary="帖子列表">
								...
								<tr>
									<td class="author"><a class="J_user_card_show" data-uid="<?php echo htmlspecialchars($value['created_userid'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=space&uid=', rawurlencode($value['created_userid']); ?>"><img class="J_avatar" src="<?php echo htmlspecialchars(Pw::getAvatar($value['created_userid'], 'small'), ENT_QUOTES, 'UTF-8');?>" data-type="small" width="45" height="45" alt="<?php echo htmlspecialchars($value['created_username'], ENT_QUOTES, 'UTF-8');?>的头像" /></a></td>
									<td class="subject">
										<p class="title">
											<?php  if ($operateThread) { ?>
											...
											<a href="<?php echo Wekit::app()->baseUrl,'/','read.php?tid=', rawurlencode($value['tid']),'&fid=', rawurlencode($value['fid']); ?>" class="st" style="<?php echo htmlspecialchars($value['highlight_style'], ENT_QUOTES, 'UTF-8');?>"><?php echo htmlspecialchars(Pw::substrs($value['subject'],$numofthreadtitle), ENT_QUOTES, 'UTF-8');?></a>
											<?php
PwHook::display(array($threadList, "runDo"), array('createHtmlAfterSubject',$value), "", $__viewer);
  ...
  if ($value['ifupload']) { ?><span class="posts_icon"><i class="icon_<?php echo htmlspecialchars($uploadIcon[$value['ifupload']], ENT_QUOTES, 'UTF-8');?>" title="<?php echo htmlspecialchars($icon[$uploadIcon[$value['ifupload']]], ENT_QUOTES, 'UTF-8');?>"></i></span><?php  } ?>
										...
										<p class="info">
											...
											最后回复：<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=space&uid=', rawurlencode($value['lastpost_userid']); ?>"><?php echo htmlspecialchars($value['lastpost_username'], ENT_QUOTES, 'UTF-8');?></a><span><a href="<?php echo Wekit::app()->baseUrl,'/','read.php?tid=', rawurlencode($value['tid']),'&fid=', rawurlencode($value['fid']),'&page=e'; ?>#a" rel="nofollow"><?php  echo Pw::time2str($value['lastpost_time'], 'm-d H:i');?></a></span>
										</p>
									</td>
									<td class="num">
										<span><em><?php echo htmlspecialchars($value['replies'], ENT_QUOTES, 'UTF-8');?></em>回复</span>
										<span><em><?php echo htmlspecialchars($value['hits'], ENT_QUOTES, 'UTF-8');?></em>浏览</span>
									</td>
								</tr>
								<?php  } ?>
							</table>
						...
						<div class="management mb10 J_post_manage_col" data-role="list">
							...
							<?php  if ($operateThread['topped']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=topped'; ?>" class="btn">置顶</a><?php  } 
  if ($operateThread['digest']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=digest'; ?>" class="btn">精华</a><?php  } 
  if ($operateThread['highlight']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=highlight'; ?>" class="btn">加亮</a><?php  } 
  if ($operateThread['up']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=up'; ?>" class="btn">提前</a><?php  } 
  if ($operateThread['type']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=type'; ?>" class="btn">分类</a><?php  } 
  if ($operateThread['move']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=move'; ?>" class="btn">移动</a><?php  } 
  if ($operateThread['copy']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=copy'; ?>" class="btn">复制</a><?php  } 
  if ($operateThread['unite']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=unite'; ?>" class="btn">合并</a><?php  } 
  if ($operateThread['lock']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=lock'; ?>" class="btn">锁定</a><?php  } 
  if ($operateThread['down']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=down'; ?>" class="btn">压帖</a><?php  } 
  if ($operateThread['delete']) { ?><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=delete'; ?>" class="btn">删除</a><?php  } ?>
						</div>
						<?php  } 
  } else { ?>
...
?>
	<a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=cate&fid=', rawurlencode($fid),'&page=', rawurlencode($_page_i); 
...
<!--design role="segment" id="linkdemo"/-->
  ...
  if ($operateThread['type']) { ?><li><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=type'; ?>">分类</a></li><?php  } ?>
								<!-- <li><a href="<?php echo Wekit::app()->baseUrl,'/','index.php?m=bbs&c=manage&a=topped'; ?>">印戳</a></li>  -->
